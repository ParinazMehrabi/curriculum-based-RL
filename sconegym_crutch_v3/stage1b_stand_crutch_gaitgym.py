...
"""
 Stage 1b (static standing WITH crutch
load-sharing reward).
"""
...
class RajagopalCrutchStage1bStandGym(GaitGym):
    ACTUATOR_NAMES = (
        "lumbar_extension",
        "hip_flexion_r",
        "knee_angle_r",
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.curriculum_stage = "1b-STAND-CRUTCH"
        ...
        # for API test
        # self._try_autowire_crutch_force_fn()
    ...
    def _try_autowire_crutch_force_fn(self):
        try:
            bodies = {self._name_of(b): b for b in self.model.bodies()}
        except Exception:
            return

        cane_bodies = [
            b for name, b in bodies.items()
            if any(k in name.lower() for k in ("cane", "crutch"))
        ]
        if len(cane_bodies) < 1:
            return

        candidate_methods = ("contact_force", "reaction_force", "grf")
        working_bodies = []
        for b in cane_bodies:
            for m in candidate_methods:
                if hasattr(b, m):
                    working_bodies.append((b, m))
                    break

        if len(working_bodies) != len(cane_bodies):
            return

        def _fn():
            total = 0.0
            for b, m in working_bodies:
                val = getattr(b, m)()
                total += abs(getattr(val, "y", val))
            return total

        self._crutch_force_fn = _fn

    def _get_crutch_contact_force(self):

        total_force = 0.0

        for name in ("Crutch_r", "Crutch_l"):

            body = None

            try:
                body = self.model.find_body(name)
    ...
    def _crutch_term(self):
        if self._crutch_force_fn is None:
            if not self._crutch_warned and self.crutch_reward_coeff != 0.0:
                print(
                    "[RajagopalCrutchStage1bStandGym] WARNING: "
                    "crutch_reward_coeff is nonzero but no crutch contact "
                    "force API has been wired up/confirmed yet. Run "
                    "find_crutch_contact_api.py and report the output "
                    "before trusting any nonzero 'crutch' reward numbers. "
                    "This component is currently forced to 0.0."
                )
                self._crutch_warned = True
            return 0.0
