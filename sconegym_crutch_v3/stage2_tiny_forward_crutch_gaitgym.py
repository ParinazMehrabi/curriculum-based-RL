...
class RajagopalCrutchStage2TinyForwardGym(GaitGym):
    ACTUATOR_NAMES = (
        "lumbar_extension",
        "hip_flexion_r",
        "knee_angle_r",
    ...
    def __init__(
        ...
        backward_penalty_coeff: float = 0.15,
        ...
        initial_forward_velocity: float = 0.02,
        ...
        **kwargs,
        ...
        self.curriculum_stage = "2-TINY-FORWARD-CRUTCH"
        ...
        if self.target_vel_v3 <= 0.0:
            raise ValueError(
                "target_vel must be positive for the tiny-forward stage "
                "(use the Stage 1b class for target_vel == 0 / pure "
                "standing)."
            )
    ...
    def _crutch_term(self):
        ...
        error = abs(
            frac - self.cane_target_load_fraction
        )

        sigma = max(
            self.cane_load_sigma_fraction,
            1e-6
        )

        return float(
            np.exp(-(error / sigma) ** 2)
        )
